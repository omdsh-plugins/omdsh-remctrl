 ...
 *
 * Two sources, one switch — the shape `omdsh-chatmode` and `omdsh-desktop` use
 ...
 * @module @omdsh-plugins/omdsh-remctrl/scripts/harness-source
 */
...
/**
 * Every harness package this plugin builds against, and where it lives in a
 * checkout.
 *
 * All of them are the BROWSER half's. The host half — the listener, the gates,
 * the pairing — imports no harness package at all, which is what lets
 * `pnpm install && pnpm test` pass on a bare clone with no harness in sight.
 */
...
function registrySpecifier(name: string): string {
  return OFF_TRAIN_VERSIONS[name] ?? HARNESS_VERSION
}
 ...
 * FAILS: unlike a runtime pin, a link here breaks `prepare` for everyone
 * installing this plugin by git URL.
 */
