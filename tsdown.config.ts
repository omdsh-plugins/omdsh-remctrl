 ...
 * half ships:
 *
 * - `lib/index.js` + `lib/contract.js` — the NODE half, bundled from the
 *   JavaScript tsc already wrote to `lib/types`, and imported by the host
 *   Loader through the row in `cordis.patch.yml`. It carries the listener, the
 *   gates, and the loopback control channel.
 ...
 *
 * `contract.js` is a second node entry rather than something consumers reach
 * through the root, so a package that only wants the wire vocabulary — a test
 * bench, a second shell — can have it without pulling in a listener.
 */
...
const browserHalf: UserConfig = {
  ...
  plugins: [{
    ...
    //
    // `@omdsh-plugins/*` is checked alongside `@deepseek-ai/*` and admits
    // NOTHING: this half type-imports the hub for its slot declaration, and a
    // day when that becomes a value import is a day this build should fail.
    name: 'dsh-client-bundle-purity',
    resolveId(source: string) {
      if (source.startsWith('@omdsh-plugins/') && source !== ID && !source.startsWith(`${ID}/`)) {
        throw new Error(
          `client bundle purity: "${source}" is another omdsh plugin — cross-plugin value imports are forbidden; `
          + 'collaborate through cordis services and slots, and keep the import type-only',
        )
      }
    ...
    // hashed class map; any other `.css` import yields an empty map and exists
    // only for its injected <style data-plugin> tag, which the loader removes
    // on unload.
    name: 'dsh-css-inline',
    resolveId(source: string, importer: string | undefined) {
      if (!source.endsWith('.css')) return null
      if (importer === undefined || source.startsWith('\0')) return CSS_VIRTUAL_PREFIX + source + CSS_VIRTUAL_SUFFIX
      // A relative specifier joins the importer's directory; a bare one goes
      // through node resolution from it.
